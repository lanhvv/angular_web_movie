export class PathConstant {
  public static readonly LOGIN = "/authentication/login";
  public static readonly REGISTER = "/authenticationregister";
  public static readonly LOGOUT = "/authentication/logout";

  public static readonly ROOT_ADMIN = "manager_web_movie";
  // ADMIN
  public static readonly DASH_BROAD = "dash_broad";
  public static readonly MOVIE = "movie";
  public static readonly CATEGORIES = "categories";
  public static readonly ADMIN = "admin";
  public static readonly USER = "user";
  public static readonly LIST = "list";
  public static readonly CAU = "c_a_u"
  //
  public static readonly ROOT_USER = "web_movie";
  // USER
  public static readonly HOME_PAGE = "home_page";
  //
}
